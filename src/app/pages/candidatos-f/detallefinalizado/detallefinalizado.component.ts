...
export class DetallefinalizadoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const localId = localStorage.getItem('uid');
      ...
      if (localId !== 'nuevo') {
       this.auth.getUsuarioPerfil(localId).subscribe((resp: UsuarioModel) => {
       this.usuario = resp;
  ...
  guardar(form:NgForm){
    ...
    BotonContrato(){
      ...
      const UidUsu=localStorage.getItem('uid');
      this.auth.getUsuarioPerfil(UidUsu)
      .subscribe(res=>{
          ...
          if(this.usuario.rol==='Admin'){
            if(this.candidato.regproceso === '17-Kit paseador entregado'){
              ...
              var ano = fecha.getFullYear(); //obteniendo año
                ...
                this.paseador.contratado_por=res['nombre'];              
              ...
              task.child("contratado_por").set(this.paseador.contratado_por);
              ...
              this.router.navigateByUrl('/paseadores');
            });
            } else {
              Swal.fire({
            ...
            Swal.fire({
              title:'Este usuario no es administrador',
              text:'Tu rol no es el indicado',
              icon:'error'
              });
          }
        }else {
          Swal.fire({
            title:'Este usuario no esta activo',
            text:'Porfavor activar esta cuenta',
            icon:'error'
            });
        }
      });
