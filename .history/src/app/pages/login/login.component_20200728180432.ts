...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  login(form: NgForm) {
    ...
    localStorage.setItem('emailVerified', resp['emailVerified']);

    this.auth.perfilUsuario(this.usuario)
      .subscribe(resp => {
        console.log('segundo', resp);
      this.usuario.email = resp.email;
      this.usuario.emialVerified = resp.emailVerified;
      ...
      if (resp.emailVerified == false){
        ...
        var emailVerified=localStorage.getItem('emailVerified');
        console.log('hola estoy activo', emailVerified);
